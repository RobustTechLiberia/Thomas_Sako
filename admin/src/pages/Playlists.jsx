...
const EMPTY = { title: "", embedUrl: "", status: "draft" };
...
export default function Playlists() {
  ...
  const load = async () => {
    try {
      ...
      const data = await api(`/playlists?${q}`);
      setItems(data.rows || []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await api(`/playlists/${editing}`, { method: "PUT", body: form });
        setMsg("Playlist updated");
      } else {
        await api("/playlists", { method: "POST", body: form });
  ...
  const publish = async (id) => {
    try {
      await api(`/playlists/${id}/publish`, { method: "POST" });
      setMsg("Playlist published — subscribers will be notified");
      load();
    } catch (err) {
      setError(err.message);
    }
  ...
  const badge = (s) => {
    const m = { published: "badge-green", draft: "badge-gray", archived: "badge-purple" };
  ...
  return (
    <>
      ...
      <div className="card">
        <h2 className="card-title">{editing ? "Edit playlist" : "Add playlist"}</h2>
        <form onSubmit={handleSubmit} className="form-grid">
          ...
          <div className="full field">
            <label>Embed / YouTube playlist URL</label>
            <input
              value={form.embedUrl}
